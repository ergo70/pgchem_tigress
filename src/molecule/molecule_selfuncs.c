 ...
 */
#define DEFAULT_MOL_MATCH_SEL 0.005
...
static Selectivity molquerysel(VariableStatData *vardata, Datum constval);
...
/*
 *	molecule_matchsel -- Selectivity of "<= =>"
 *
 * restriction selectivity function for molecule >= molecule and
 * molecule <= molecule
 */
Datum
molecule_matchsel(PG_FUNCTION_ARGS)
{
	...
	if (!get_restriction_variable(root, args, varRelid,
		...
		PG_RETURN_FLOAT8(DEFAULT_MOL_MATCH_SEL);
